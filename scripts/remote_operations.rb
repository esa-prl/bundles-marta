...
OptionParser.new do |opts|
    opts.banner = "Usage: ruby marta_remote_operations.rb [options]"
...
Bundles.initialize
Bundles.transformer.load_conf(Bundles.find_file('config', 'transforms_scripts_marta.rb'))

Orocos::Process.run 'control', 'pancam_bb3', 'navcam_bb3', 'loccam', 'imu', 'tmtchandling', 'unit_vicon', 'navigation', 'unit_visual_odometry', 'fdir', 'autonomy' do
    ...
    #Orocos.conf.apply(imu_stim300, ['default', 'marta', 'ESTEC', 'stim300_5g'], :override => true)
    ...
    if options[:pan]
        puts "Configuring PanCam"

        camera_firewire_pancam = TaskContext.get 'camera_firewire_pancam'
        Orocos.conf.apply(camera_firewire_pancam, ['hdpr_bb3', 'altec_bb3_id','auto_exposure'], :override => true)
        ...
        Orocos.conf.apply(stereo_pancam, ['marta_bb3_left_right'], :override => true)
    ...
    Orocos.conf.apply(waypoint_navigation, ['default','marta'], :override => true)
    ...
    if options[:autonav]
        ...
        Orocos.conf.apply(autonav, ['marta_navcam'], :override => true)
    ...
    if options[:nav]
        ...
        if options[:autonav]
            # might need to check timestamps of left & right images (must be the same)
            camera_navcam.left_frame.connect_to             autonav.frame_left
            camera_navcam.right_frame.connect_to            autonav.frame_right
        end
